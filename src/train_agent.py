...
parser.add_argument("--dfa-samplers-eval", default=None, nargs='+',
                    help="the ltl formula templates to sample from for evaluation (default: use the same \"dfa-sampler\" as training)")
...
parser.add_argument("--freeze", action="store_true", default=False,help="Freeze the gradient updates of the LTL module")
...
if args.freeze:
    gnn_name = gnn_name + "-freeze"
if use_mem:
    gnn_name = gnn_name + "-recurrence:%d"%(args.recurrence)

default_model_name = f"{gnn_name}_{args.dfa_sampler}_{args.env}_seed:{args.seed}_epochs:{args.epochs}_bs:{args.batch_size}_fpp:{args.frames_per_proc}_dsc:{args.discount}_lr:{args.lr}_ent:{args.entropy_coef}_clip:{args.clip_eps}_prog:{args.progression_mode}_dfa:{args.dfa}"
...
if args.pretrained_gnn:
    ...
    default_dir = f"storage/{args.gnn}-dumb_ac_{args.pretrained_gnn_sampler}_Dummy-MDP-v0_seed:{args.seed}_*_prog:{args.progression_mode}_dfa:{args.dfa}/train"
    print(default_dir)
...
for i in range(args.procs):
    envs.append(utils.make_env(args.env, progression_mode, args.dfa_sampler, args.seed, args.int_reward, args.noLTL, args.dfa))
...
# Load model
if use_mem:
    acmodel = RecurrentACModel(envs[0].env, obs_space, envs[0].action_space, args.ignoreLTL, args.gnn, args.dumb_ac, args.freeze)
else:
    acmodel = ACModel(envs[0].env, obs_space, envs[0].action_space, args.ignoreLTL, args.gnn, args.dumb_ac, args.freeze)
...
# init the evaluator
if args.eval:
    eval_samplers = args.dfa_samplers_eval if args.dfa_samplers_eval else [args.dfa_sampler]
